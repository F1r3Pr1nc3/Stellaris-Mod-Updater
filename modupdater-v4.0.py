...
def parse_arguments():
	...
	parser.add_argument('-output', '--mod_outpath', type=str, default="",
						help='Output path for the updated mod')
...
v4_0 = {
	...
	"targets3": {
		...
		r"(\s+)job_(?:calculator|brain_drone)_add\s*=\s*(-?[1-9])\b\n?": lambda m:
		...
		r"\bplanet_immigration_pull_(mult|add)\s*=\s*(-?[\d.]+)": lambda m: f"planet_resettlement_unemployed_destination_{m.group(1)} = {float(m.group(2))*2}",
	...
	"targets4": {
		...
		r"\bpop_produces_resource = \{\s+[^{}#]+\}": [r"\(bpop_produces_resource) = \{\s+(type\s*=\s*\w+)\s+(amount\s*[<=>]+\s*[^{}\s]+)\s+\}", r"# \1= { \2 \3 }"], # Comment out
...
def extract_scripted_triggers(mod_path):
	"""
...
def apply_merger_of_rules(mod_path):
	...
	merger_triggers = {
		...
		"merg_is_gaia_basic": (r"\b(is_planet_class = pc_gaia|pd_is_planet_class_gaia)\b", (no_trigger_folder, r"merg_is_gaia_basic = yes")),
	...
	if mergerofrules:
		for trigger in merger_triggers:
			if len(merger_triggers[trigger]) == 3:
				targets4[merger_triggers[trigger][0]] = merger_triggers[trigger][1]
			else:
				targets3[merger_triggers[trigger][0]] = merger_triggers[trigger][1]
		if not keep_default_country_trigger:
			targets4[
		...
		triggers_in_mod = extract_scripted_triggers(mod_path)
		merger_reverse_triggers = {
			...
			"merg_is_hive": ( r"\bmerg_is_(hive)_world = (yes|no)", merg_planet_rev_lambda ),
			"merg_is_relic": ( r"\bmerg_is_(relic)_world = (yes|no)", merg_planet_rev_lambda ),
			"merg_is_machine": ( r"\bmerg_is_(machine)_world = (yes|no)", merg_planet_rev_lambda ),
...
def parse_dir():
	...
	apply_merger_of_rules(mod_path)
